...
function installVPN(){
	...
	echo "localip 192.168.100.100" >> /etc/pptpd.conf
	echo "remoteip 192.168.100.101-110" >> /etc/pptpd.conf
	...
	iptables -t nat -A POSTROUTING -s 192.168.100.100 -j SNAT --to-source `ifconfig  | grep 'inet addr:'| grep -v '127.0.0.1' | cut -d: -f2 | awk 'NR==1 { print $1}'`
